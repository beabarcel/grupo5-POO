...
import br.com.poo.bancoAmbl3.enums.Gerente;
...
public class JGerente extends JFrame {
	...
	public JGerente() {
		...
		List<Gerente> funcoesP = Arrays.asList(Gerente.values());
		comboBox.addItem("Selecione uma das opções:");
		for(Gerente opcao : funcoesP) {
			comboBox.addItem(opcao.getTipo());
		}
		comboBox.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String tipo = comboBox.getSelectedItem().toString();
				if(tipo.equalsIgnoreCase(Gerente.CADASTRAR_CONTA.getTipo())) {
					//dispose();
					JCadastro jCadastro = new JCadastro();
					jCadastro.setLocationRelativeTo(jCadastro);
					jCadastro.setVisible(true);
				} else if(tipo.equalsIgnoreCase(Gerente.RELATORIO_CONTAS_AGENCIA.getTipo())) {
					dispose();
					//relatorio
				} else if(tipo.equalsIgnoreCase(Gerente.RELATORIO_CONTAS.getTipo())) {
					dispose();
					//relatorio
				}
			}
